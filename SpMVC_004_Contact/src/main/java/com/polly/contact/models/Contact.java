...
public class Contact {
	 ...
	 * 이 변수들을 '인스턴스 변수'라고 한다.
	 * 인스턴스 변수는 클래스를 사용하여 객체를 선언하고
	 * 생성자(new) 를 사용하여 초기화되어 객체가 인스턴스화 할 때
	 * 변수도 함께 사용할 준비가 된다.
	 * 
	 ...
	 * 생성자(생성자 method)
	 * 클래스를 사용하여 객체를 선언하고
	 * 객체를 초기화(인스턴스화) 할 때 사용(호출)하는 method
	 * 
	 * 모든 클래스는 클래스를 선언하면 자동으로 기본 생성자가 만들어진다.
	 * 다만 코드로 보이지 않을 뿐이다.
	 * 기본 생성자는 매개변수가 없고
	 * 		인스턴스 변수들을 기본값(null, 0...)으로 초기화하는 코드가
	 * 		내부적으로 작성되어 있다.
	 * 
	 ...
	 * '임의 생성자를 만들 때는 기본 생성자도 만들어주는 것이 좋다'.
	 * 그래야만 기본 생성자를 사용하는 코드에서 오류가 발생하지 않는다.
	 */
	
	// 마우스 우클릭 -> source -> Generate Constructors from Superclass...	
	// 기본 생성자(매개변수 X)
	public Contact() {
		super();
