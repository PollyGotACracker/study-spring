 ...
 * DB 로부터 결과를 받아서 Dto(VO) 객체에 담는 일을 
 * 수행하는 클래스
 * 
 * interface: 일반 class 와 다른 특징
 * 		method 를 정의(prototype, 모양, 구조)만 하고 실제 코드는 없다.
 * 		interface 를 기반으로 작성된 클래스(implements) 를
 * 		interface 의 구현체 클래스라고 하기도 한다.
 * 
...
public interface BookDao {
	/**
	 * 전체 데이터를 select 하여
	 * 그 결과(table) 을 List<BookDto> 형태로 변환하여 return
	 */
//	@Select("SELECT * FROM tbl_books")
	public List<BookDto> selectAll();
	
	/**
	 * isbn 값을 파라미터로 전달하여 한 개의 도서정보를 select 하고
	 * 그 결과를 BookDto 형태로 변환하여 return
	 */
	@Select("SELECT * FROM tbl_books WHERE isbn = #{isbn}")
	public BookDto findByIsbn(String isbn);
	
	/**
	 * BookDto 형태로 저장된 데이터를 파라미터로 전달하여
	 * DB table 에 insert 를 수행하고 그 결과를 int 형으로 return
	 * 몇 개의 데이터가 insert 되었는가를 return
	 */
	 ...
	 * 다소 복잡하다.
	 * SQL 코드가 복잡해지면 Annotation 기반으로 Query 를 작성하기가 불편해진다.
	 * 
	 * 이럴 때는 mybatis mapper 를 연동하여 SQL 을 처리할 수 있다.
