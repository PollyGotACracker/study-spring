...
public class HomeController {
	 ...
	 * WEB-INF/views/ 폴더에 저장된 JSP 파일을 rendering 하여 response 한다.
	 */
	...
	public BookDto getMyBook() {
		 ...
		 * 전통적인 객체지향 언어(POJO, Plan Old Java Object)이다보니
		 * 클래스를 사용하여 객체를 생성하는 데 다소 복잡한 규칙이 적용된다.
		 * 
		 * 1. 클래스를 이용하여 객체를 선언하고,
		 * 2. 생성자를 호출하여 객체를 초기화하고,
		 * 3. 초기화된 객체의 setter method 를 사용하여
		 * 4. 각 속성(멤버변수)의 값을 설정하는 방법을 사용하거나
		 * 
		 * 1. 필드생성자를 만들고
		 * 2. 생성자를 호출할 때 초기값을 전달하여 
		 * 3. 객체로 생성하는 방법
		 * 
		 * lombok 도구를 활용하면 클래스를 객체로 생성하는 데
		 * builder 디자인 패턴을 사용할 수 있다.
		 * 
		 * 1. dto(vo) 클래스에 @Builder Annotation 을 부착하고
		 * 2. 객체를 생성할 때 클래스.builder() method 를 먼저 호출하여
		 * 3. 객체를 초기화 하고
		 * 4. 각 멤버 변수 이름을 직접 활용하여
		 * 5. 값을 setting 하고
		 * 6. build() method 를 호출하여 객체를 완전 생성하는 구조
		 * 
		 * 필요한 멤버변수 값만 초기화 할 수 있고
		 * 정확히 어떤 멤버변수를 초기화하는지 개발자가 명확히 알 수 있다.
		 */
		...
		// 아래처럼 2개의 멤버변수만 초기화하는 생성코드를 사용하려면
		// 그에 맞는 생성자 method 가 준비되어야 한다.
